...
function localTimeParts(
  timezone: string,
  now: Date,
): { date: string; minuteOfDay: number } | null {
  try {
    ...
    if (
      !values.year || !values.month || !values.day || Number.isNaN(hour) ||
      Number.isNaN(minute)
    ) {
      return null;
    }
...
function duePreferences(
  ...
  for (const preference of preferences) {
    ...
    const offsets = Array.isArray(preference.reminder_offsets)
...
serve(async (req) => {
  ...
  const supportEmail = Deno.env.get("SUPPORT_EMAIL") ??
    "its.5pm.somewhere.app@gmail.com";
  ...
  const { data: preferences, error: preferenceError } = await admin
    ...
    .select(
      "id, user_id, city_id, city_name, country_code, timezone, reminder_offsets",
    )
    .eq("active", true)
    .limit(5000);
  ...
  if (preferenceError) {
    console.error(
      "send-5pm-notifications: preference query failed",
      preferenceError,
    );
    return jsonResponse({ error: "Could not load preferences." }, 500);
  }

  const due = duePreferences(
  ...
  if (subscriptionError) {
    console.error(
      "send-5pm-notifications: subscription query failed",
      subscriptionError,
    );
  ...
  for (const preference of due) {
    ...
    for (const subscription of userSubscriptions) {
      const { error: claimError } = await admin.from(
